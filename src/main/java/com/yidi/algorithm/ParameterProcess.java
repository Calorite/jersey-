...
public class ParameterProcess {
	...
	private static Map<Integer,Parameter> allparamenter=null;
	...
	public Set<Parameter> getParemetes(String text) throws SQLException{
		allparamenter=DBupdate.getparams();
		Set<Parameter> targetpara=new HashSet<Parameter>();
		for(Parameter curtparamente:allparamenter.values()) {
			if(checkParamete(curtparamente.getParameter(),text)) {
	...
	public List<String> returnquesid(Map<Set<Integer>, Integer> parameterlist,Set<Parameter> parametes) throws SQLException {//返回questionid或者solutionid
		List<String> returnlist=new LinkedList<String>();
		Set<Integer> leave=new HashSet<Integer>();
		Set<Integer> getedpara=new HashSet<Integer>();
		for(Parameter curpara:parametes) {
			getedpara.add(curpara.getParameterid());
		}
		for(Set<Integer> set:parameterlist.keySet()) {
